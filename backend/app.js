...
const MY_JWT_SECRET = env.JWT_SECRET;
...
app.post('/api/reg', async function (req, res) {
    ...
    try {
        ...
        console.log(error);
        // const errorInstance = new SQLInsertError(error as string);
...
app.post('/api/auth', async function (req, res) {
    ...
    const pHash = createHash('sha256').update(password).digest('hex');
    const requestUser = new Promise((resolve, reject) => {
        db.all('select id, login from users where login=(?) and password=(?)', [username, pHash], (err, rows) => {
            if (err)
                reject(err);
            if (rows.length === 0) {
...
app.put('/api/todo', async function (req, res) {
    if (!req.body)
        return res.sendStatus(400);
    let todo, titleToUpdate, isDoneToUpdate;
